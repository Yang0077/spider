...
class DictClassifier:
    def __init__(self):
        self.__root_filepath = "analyze/dict/"

        jieba.load_userdict("analyze/dict/user_dict.txt")  # 准备分词词典
    ...
    def analyse_clause(self, the_clause, runout_filepath, print_show):
        ...
        if runout_filepath is not None:
            self.write_run_out_file(runout_filepath, the_clause + '\n')
            self.write_run_out_file(runout_filepath, str(seg_result) + '\n')
        if print_show:
            print(the_clause)
            print(seg_result)
    ...
    def output_analysis(self, comment_analysis, runout_filepath=None):
        ...
        for i in range(len(comment_analysis) - 1):
            ...
            output += "\n"
        if runout_filepath is not None:
            self.write_run_out_file(runout_filepath, output)
        else:
            print(output)
    ...
    @staticmethod
    def write_run_out_file(path, info, encoding="utf-8"):
...
if __name__ == '__main__':
    obj = DictClassifier()
    a_sentence = "电脑很快就收到了性价不行非常不好用电脑整体效果不太行颜值很低"
