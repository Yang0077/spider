...
def run_spider_handler(*args):
    try:
        ...
        # 爬取数据
        ret_data = start_jd_spider(global_driver)
...
def init_GUI():
    ...
    # 京东登录按钮
    button = Button(root_window, width=20, text="登录京东商城", command=lambda: thread_it(on_login_button_click))
    ...
    Label(root_window, textvariable=lable_value, font=('微软雅黑', 12)).grid(row=2, column=0, padx=22, pady=10)
